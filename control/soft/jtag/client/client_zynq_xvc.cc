...
int shiftJtag(int sockfd, char* tms, char* tdi, char* tdo, int32_t length)
{
  int n;
  char* buf;
  ...
  n = write(sockfd,shiftcmd.c_str(),strlen(shiftcmd.c_str()));
  if (n < 0)
    {
  ...
  n = write(sockfd,buf,num_bytes*2);
  if (n < 0)
    {
      error("ERROR writing to socket");
      return -1;
